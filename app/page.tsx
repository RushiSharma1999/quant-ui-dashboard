...
// Set in .env.local / .env.production
const PRESIGN_URL = process.env.NEXT_PUBLIC_UPLOAD_PRESIGN_URL as string | undefined
  ...
  async function uploadFile(file: File) {
    ...
    if (!PRESIGN_URL) {
      toast({
        ...
        description: "Set NEXT_PUBLIC_UPLOAD_PRESIGN_URL and rebuild.",
        variant: "destructive",
      })
      return
    }
    ...
    try {
      ...
      // 1) Ask Lambda for pre-signed URL (adjust method if your Lambda expects POST)
      const qs = new URLSearchParams({
        filename: file.name,
        contentType: file.type || "text/csv",
      })
      const res = await fetch(`${PRESIGN_URL}?${qs.toString()}`, { method: "GET" })
      if (!res.ok) {
        const body = await res.text().catch(() => "")
        console.error("Presign failed", res.status, body)
      ...
      // 2) PUT the file to S3 with progress
      await putWithProgress(url, file, (p) => setUploadProgress(p))

      setIsUploading(false)
      setUploadProgress(100)
      toast({
        title: "Upload successful",
        description: `File uploaded${key ? ` as ${key}` : ""}.`,
      })
    } catch (err: any) {
      setIsUploading(false)
      setUploadProgress(0)
                ...
                <Card className="border-green-200 bg-green-50/50">
                  <CardHeader>
                    ...
                    <CardDescription className="text-green-600">
                      Import CSV files to update inventory
                    </CardDescription>
                  </CardHeader>
